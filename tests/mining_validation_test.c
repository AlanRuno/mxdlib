...
static void test_mining_validation(void) {
    ...
    TEST_ASSERT(mxd_create_transaction(&genesis_tx) == 0, "Genesis transaction creation");
    TEST_ASSERT(mxd_add_tx_output(&genesis_tx, public_key, 1000.0) == 0, "Genesis output addition");
    TEST_ASSERT(mxd_calculate_tx_hash(&genesis_tx, genesis_hash) == 0, "Genesis hash calculation");
    ...
    for (int i = 0; i < TEST_TRANSACTIONS; i++) {
        ...
        TEST_ASSERT(mxd_add_tx_input(&transactions[i], genesis_tx.tx_hash, 0, public_key) == 0,
                   "Input addition");
        ...
        for (size_t j = 0; j < TEST_NODE_COUNT; j++) {
            ...
            if (validation_result != 0) {
                ...
                TEST_ASSERT(mxd_update_node_metrics(&nodes[j],
                    validation_time, validation_end) == 0,
                    "Metrics update");
            }
        }
    ...
    for (size_t i = 1; i < TEST_NODE_COUNT; i++) {
        TEST_ASSERT(nodes[i-1].metrics.avg_response_time <= nodes[i].metrics.avg_response_time,
                   "Node ranking order");
    }
    ...
    for (size_t i = 0; i < TEST_NODE_COUNT; i++) {
        ...
        // Add minimum required responses (10) with response time under MXD_MAX_RESPONSE_TIME
        for (int j = 0; j < MXD_MIN_RESPONSE_COUNT; j++) {
            ...
            TEST_ASSERT(mxd_update_node_metrics(&nodes[i], response_time, 
                current_time + j * 1000) == 0, "Metrics update");
        }
        
        // Validate node performance after stake validation
        TEST_ASSERT(mxd_validate_node_performance(&nodes[i], current_time + 10000) == 0,
...
int main(void) {
    ...
    // Initialize NTP and RSC systems
