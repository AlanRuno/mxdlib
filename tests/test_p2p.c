...
static void *echo_server_thread_func(void *arg) {
    ...
    // Set socket options
    int opt = 1;
    assert(setsockopt(echo_server_socket, SOL_SOCKET, SO_REUSEADDR,
                     &opt, sizeof(opt)) >= 0);
...
static void test_message_handling(void) {
    // Start echo server
    assert(pthread_create(&echo_server_thread, NULL,
                         echo_server_thread_func, NULL) == 0);
    ...
    assert(mxd_send_message("127.0.0.1", 12347, MXD_MSG_PING,
                           test_message, strlen(test_message)) == 0);
    
    // Broadcast test message
    assert(mxd_broadcast_message(MXD_MSG_PING,
                               test_message, strlen(test_message)) == 0);
