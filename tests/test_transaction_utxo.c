...
static void setup_test_keys(void) {
  mxd_generate_public_key(test_private_key, test_public_key);
  mxd_generate_public_key(test_private_key2, test_public_key2);
}
...
static void test_transaction_validation_with_utxo(void) {
  ...
  mxd_transaction_t *tx = mxd_create_transaction();
  TEST_ASSERT(tx != NULL, "Create transaction");
  ...
  TEST_ASSERT(mxd_calculate_tx_hash(tx) == 0, "Calculate transaction hash");
...
static void test_double_spend_prevention(void) {
  ...
  mxd_transaction_t *tx1 = mxd_create_transaction();
  TEST_ASSERT(tx1 != NULL, "Create first transaction");
  ...
  TEST_ASSERT(mxd_calculate_tx_hash(tx1) == 0, "Calculate first transaction hash");
  ...
  mxd_transaction_t *tx2 = mxd_create_transaction();
  TEST_ASSERT(tx2 != NULL, "Create second transaction");
  ...
  TEST_ASSERT(mxd_calculate_tx_hash(tx2) == 0, "Calculate second transaction hash");
