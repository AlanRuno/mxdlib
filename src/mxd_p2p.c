...
int mxd_init_p2p(uint16_t port) {
  ...
  // Set socket options
  int opt = 1;
  if (setsockopt(p2p_state.server_socket, SOL_SOCKET, SO_REUSEADDR, &opt,
                 sizeof(opt)) < 0) {
  ...
  if (flags < 0 ||
      fcntl(p2p_state.server_socket, F_SETFL, flags | O_NONBLOCK) < 0) {
    close(p2p_state.server_socket);
    pthread_mutex_destroy(&p2p_state.peers_mutex);
    free(p2p_state.peers);
    return -1;
...
int mxd_start_p2p(void) {
  ...
  // Start server thread
  if (pthread_create(&p2p_state.server_thread, NULL, server_thread_func,
                     NULL) != 0) {
    p2p_state.running = 0;
    return -1;
  }
  ...
  if (pthread_create(&p2p_state.discovery_thread, NULL, discovery_thread_func,
                     NULL) != 0) {
    p2p_state.running = 0;
...
int mxd_send_message(const char *address, uint16_t port,
  ...
  mxd_message_header_t header = {.magic = 0x4D584400, // "MXD\0"
                                 .type = type,
...
int mxd_broadcast_message(mxd_message_type_t type, const void *payload,
                          size_t payload_length) {
  if (!payload || payload_length > MXD_MAX_MESSAGE_SIZE || !p2p_state.running) {
    return -1;
  }
  ...
  for (size_t i = 0; i < p2p_state.peer_count; i++) {
    if (p2p_state.peers[i].state == MXD_PEER_CONNECTED) {
      mxd_send_message(p2p_state.peers[i].address, p2p_state.peers[i].port,
