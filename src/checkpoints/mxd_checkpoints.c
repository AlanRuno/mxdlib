...
int mxd_init_checkpoints(mxd_checkpoint_manager_t *manager, size_t initial_capacity) {
...
int mxd_create_checkpoint(mxd_checkpoint_manager_t *manager,
    ...
    if (manager->count >= manager->capacity) {
        ...
        mxd_checkpoint_t *new_checkpoints = realloc(manager->checkpoints,
                                                   new_capacity * sizeof(mxd_checkpoint_t));
        if (!new_checkpoints) {
            return -1;
        }
...
int mxd_validate_checkpoint(const mxd_checkpoint_manager_t *manager,
    ...
    const mxd_checkpoint_t *last_checkpoint = &manager->checkpoints[manager->count - 1];
...
int mxd_prune_checkpoints(mxd_checkpoint_manager_t *manager,
    ...
    if (remaining > 0) {
        memmove(manager->checkpoints,
                &manager->checkpoints[keep_index],
...
int mxd_recover_from_checkpoint(const mxd_checkpoint_manager_t *manager,
                              ...
                              uint8_t *state_data, size_t *state_size) {
