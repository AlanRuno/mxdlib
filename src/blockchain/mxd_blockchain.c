...
int mxd_add_transaction(mxd_block_t *block, const uint8_t *transaction_data,
    ...
    transaction_t *new_transactions = realloc(transactions,
                                            (transaction_count + 1) * sizeof(transaction_t));
    if (!new_transactions) {
        return -1;
    }
    ...
    memcpy(transactions[transaction_count].data, transaction_data, transaction_length);
