...
int mxd_add_tx_input(mxd_transaction_t *tx,
    ...
    mxd_tx_input_t *new_inputs = realloc(tx->inputs,
                                        (tx->input_count + 1) * sizeof(mxd_tx_input_t));
    if (!new_inputs) {
        return -1;
    }
...
int mxd_add_tx_output(mxd_transaction_t *tx,
                      const uint8_t recipient_key[256],
                      double amount) {
    ...
    mxd_tx_output_t *new_outputs = realloc(tx->outputs,
                                          (tx->output_count + 1) * sizeof(mxd_tx_output_t));
    if (!new_outputs) {
        return -1;
    }
...
int mxd_calculate_tx_hash(const mxd_transaction_t *tx,
                         uint8_t hash[64]) {
    if (!tx || !hash) {
        return -1;
    }
    ...
    size_t buffer_size = sizeof(uint32_t) * 3 +  // version + counts
...
int mxd_sign_tx_input(mxd_transaction_t *tx,
...
int mxd_verify_tx_input(const mxd_transaction_t *tx,
                        uint32_t input_index) {
    ...
    return mxd_dilithium_verify(tx->inputs[input_index].signature,
                               256,
                               tx_hash, 64,
                               tx->inputs[input_index].public_key);
}

// Validate entire transaction
int mxd_validate_transaction(const mxd_transaction_t *tx) {
    if (!tx || tx->version != 1 ||
        tx->input_count == 0 || tx->input_count > MXD_MAX_TX_INPUTS ||
        tx->output_count == 0 || tx->output_count > MXD_MAX_TX_OUTPUTS) {
        return -1;
    }
